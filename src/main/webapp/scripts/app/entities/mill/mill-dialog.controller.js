...
angular.module('omsApp').controller('MillDialogController',
    ['$scope', '$stateParams', '$uibModalInstance', '$q', 'entity', 'Mill', 'Addresses', 'NoteSet', 'Price', 'Quality', 'SimpleGsmShade', 'CustomerGroup', 'DerivedGsmShade',
        function($scope, $stateParams, $uibModalInstance, $q, entity, Mill, Addresses, NoteSet, Price, Quality, SimpleGsmShade, CustomerGroup, DerivedGsmShade) {

        $scope.mill = entity;
        $scope.addressess = Addresses.query({filter: 'mill-is-null'});
        $q.all([$scope.mill.$promise, $scope.addressess.$promise]).then(function() {
            if (!$scope.mill.addresses || !$scope.mill.addresses.id) {
                return $q.reject();
            }
        ...
        $q.all([$scope.mill.$promise, $scope.notess.$promise]).then(function() {
            if (!$scope.mill.notes || !$scope.mill.notes.id) {
                return $q.reject();
            }
