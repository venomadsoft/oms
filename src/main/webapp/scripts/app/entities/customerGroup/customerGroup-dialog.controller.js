...
angular.module('omsApp').controller('CustomerGroupDialogController',
    ['$scope', '$stateParams', '$uibModalInstance', '$q', 'entity', 'CustomerGroup', 'PriceList', 'Mill', 'Customer', 'NoteSet', 'Formulae',
        function($scope, $stateParams, $uibModalInstance, $q, entity, CustomerGroup, PriceList, Mill, Customer, NoteSet, Formulae) {
        ...
        $q.all([$scope.customerGroup.$promise, $scope.notess.$promise]).then(function() {
            if (!$scope.customerGroup.notes || !$scope.customerGroup.notes.id) {
                return $q.reject();
            }
        ...
        $q.all([$scope.customerGroup.$promise, $scope.formulaes.$promise]).then(function() {
            if (!$scope.customerGroup.formulae || !$scope.customerGroup.formulae.id) {
                return $q.reject();
            }
