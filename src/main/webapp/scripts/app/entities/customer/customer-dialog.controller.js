...
angular.module('omsApp').controller('CustomerDialogController',
    ['$scope', '$stateParams', '$uibModalInstance', '$q', 'entity', 'Customer', 'Addresses', 'NoteSet', 'Formulae', 'CustomerGroup',
        function($scope, $stateParams, $uibModalInstance, $q, entity, Customer, Addresses, NoteSet, Formulae, CustomerGroup) {
        ...
        $q.all([$scope.customer.$promise, $scope.addressess.$promise]).then(function() {
            if (!$scope.customer.addresses || !$scope.customer.addresses.id) {
                return $q.reject();
            }
        ...
        $q.all([$scope.customer.$promise, $scope.notess.$promise]).then(function() {
            if (!$scope.customer.notes || !$scope.customer.notes.id) {
                return $q.reject();
            }
        ...
        $q.all([$scope.customer.$promise, $scope.formulaes.$promise]).then(function() {
            if (!$scope.customer.formulae || !$scope.customer.formulae.id) {
                return $q.reject();
            }
