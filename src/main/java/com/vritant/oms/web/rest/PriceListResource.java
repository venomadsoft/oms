...
public class PriceListResource {
    ...
    public ResponseEntity<PriceList> createPriceList(@Valid @RequestBody PriceList priceList) throws URISyntaxException {
        ...
        if (priceList.getId() != null) {
            return ResponseEntity.badRequest().header("Failure", "A new priceList cannot already have an ID").body(null);
        }
    ...
    public ResponseEntity<PriceList> getPriceList(@PathVariable Long id) {
        ...
        return Optional.ofNullable(priceListRepository.findOne(id))
            .map(priceList -> new ResponseEntity<>(
                priceList,
                HttpStatus.OK))
