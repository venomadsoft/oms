...
public class FormulaResource {
    ...
    public ResponseEntity<Formula> createFormula(@Valid @RequestBody Formula formula) throws URISyntaxException {
        ...
        if (formula.getId() != null) {
            return ResponseEntity.badRequest().header("Failure", "A new formula cannot already have an ID").body(null);
        }
    ...
    public ResponseEntity<Formula> getFormula(@PathVariable Long id) {
        ...
        return Optional.ofNullable(formulaRepository.findOne(id))
            .map(formula -> new ResponseEntity<>(
                formula,
                HttpStatus.OK))
